...
for step in range(224):
 for center in [40,64,87]:
  source=np.exp(-((X-center-5*np.sin(step*.17+center))**2/55+(Y-175)**2/13))
  t+=source*(.22+.1*rng.random());sm+=source*.12
 v-=t*.12;u+=np.sin(Y*.11+step*.14)*t*.032
...
fire.save('public/art/fluid-fire-v1.png');smoke.save('public/art/fluid-smoke-v1.png')
