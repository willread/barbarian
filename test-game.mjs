...
source=source.replace('window.ashenAxe = {','window.__test = { get hero(){return hero}, get enemies(){return enemies}, get clock(){return clock}, get spell(){return spell} }; window.ashenAxe = {');
...
press('d');press('k');step(5);assert.equal(G.status().magic,95);
assert.ok(Math.abs(T.enemies[0].hp-(enemyHP-5/3))<1e-9);
const hpBeforeTick=T.enemies[0].hp;
step(1);assert.ok(Math.abs(T.enemies[0].hp-(hpBeforeTick-1/3))<1e-9,'damage flows every tick');
...
release('d');step(18);assert.ok(Math.abs(T.enemies[0].hp-(enemyHP-24/3))<1e-9,'held spell deals steady damage');
...
press('k');step(1);assert.equal(G.status().magic,retained-1,'partial charge works');
...
magicButton.onpointerdown({pointerId:1});step(6);assert.equal(G.status().magic,94);
magicButton.onpointercancel();step(20);assert.equal(G.status().magic,94);assert.equal(G.status().channeling,false);
magicButton.onpointerdown({pointerId:2});step(6);magicButton.onlostpointercapture();step(20);
assert.equal(G.status().magic,88);assert.equal(G.status().channeling,false);
...
assert.ok(T.hero.x>escapeX);assert.ok(T.hero.invTicks>0);assert.equal(G.status().magic,99);
step(24);assert.equal(T.hero.invTicks,0,'channeling does not refresh escape protection');
release('d');release('k');
M.hurt(T.hero,{direction:-1,knock:true},true);press('k');step(1);
...
assert.equal(G.status().magic,1);step(1);assert.equal(G.status().magic,0);
...
release('k');assert.equal(T.spell,null);
