...
setlocal
cd /d "%~dp0"
echo Building Cairn Steam Shareware Edition for browser and Windows.
echo Close the Windows game before building so Cairn.exe can be replaced.
echo.
call npm run build:steam-shareware
set "cairn_build_result=%errorlevel%"
echo.
if "%cairn_build_result%"=="0" (
  echo Steam shareware build complete. Output paths are listed above.
) else (
