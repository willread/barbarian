...
export default function Home(){useEffect(()=>{
  ...
  load('/mechanics.js?v=9').then(()=>{if(!disposed)return load('/animation.js?v=9')}).then(()=>{if(!disposed)return load('/hero-rig.js?v=10')}).then(()=>{if(!disposed)return load('/blood.js?v=4')}).then(()=>{if(!disposed)return load('/enemies.js?v=2')}).then(()=>{if(!disposed)return load('/enemy-art.js?v=1')}).then(()=>{if(!disposed)return load('/enemy-rig.js?v=1')}).then(()=>{if(!disposed)return load('/game.js?v=20')}).catch(()=>{
...
<div id="stage"><canvas id="game" width="1440" height="810" aria-label="Battle arena. WASD to move, J attack, Space jump, press K at full charge to cast magic."/><div id="hud" hidden><div><small>KAEL · THE EXILE</small><div className="health"><i id="health"/></div><div id="magic"><div className="magic-label"><span>STORMCALL</span><b id="magic-state">PRESS K</b></div><div id="magic-meter" role="progressbar" aria-label="Stormcall charge" aria-valuemin={0} aria-valuemax={100} aria-valuenow={100}><i id="magic-fill"/></div><span className="magic-hint">Weapon hits +12 · Kills +20</span></div></div><div className="score"><small>SCORE</small><strong id="score">000000</strong><small id="wave">WAVE 1 / 4</small></div></div>
...
<div className="stage-bottom"><small>✧ &nbsp; THE FORGOTTEN KINGDOM</small><div><label className="weapon-choice">WEAPON <select id="weapon" aria-label="Weapon"><option value="axe">Axe · heavy</option><option value="sword">Sword · fast / long</option></select></label><button id="sound" aria-label="Enable sound">SOUND OFF</button><button id="pause" aria-label="Pause battle">Ⅱ</button><button id="full" aria-label="Fullscreen">⛶</button></div></div></div>
<div className="touch">{[['a','←'],['w','↑'],['s','↓'],['d','→'],['j','ATTACK'],[' ','JUMP'],['k','MAGIC']].map(([key,label])=><button key={key} data-key={key}>{label}</button>)}</div>
<footer><div><kbd>W A S D</kbd><p>Move<small>double-tap left/right to run</small></p></div><div><kbd>J</kbd><p>Attack<small>tap after each strike</small></p></div><div><kbd>SPACE</kbd><p>Jump<small>J in air · J + Space: back attack</small></p></div><div><kbd>K</kbd><p>Magic<small>full charge · 1.5s burst / break stun</small></p></div><div><kbd>P</kbd><p>Pause</p></div></footer><div className="footnote"><span>FORGED IN THE AGE OF SWORD & SORCERY</span><span>Headphones recommended ↗</span></div></main>}
