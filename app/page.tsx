...
export default function Home(){useEffect(()=>{
  ...
  load('/mechanics.js?v=5').then(()=>{if(!disposed)return load('/animation.js?v=7')}).then(()=>{if(!disposed)return load('/game.js?v=7')}).catch(()=>{
...
<div id="stage"><canvas id="game" width="1440" height="810" aria-label="Battle arena. WASD to move, J attack, Space jump, K magic."/><div id="hud" hidden><div><small>KAEL · THE EXILE</small><div className="health"><i id="health"/></div><div id="magic"><div className="magic-label"><span>STORMCALL</span><b id="magic-state">K · READY</b></div><div id="magic-meter" role="progressbar" aria-label="Stormcall charge" aria-valuemin={0} aria-valuemax={100} aria-valuenow={100}><i id="magic-fill"/></div><span className="magic-hint">Axe hits +12 · Kills +20</span></div></div><div className="score"><small>SCORE</small><strong id="score">000000</strong><small id="wave">WAVE 1 / 4</small></div></div>
...
<footer><div><kbd>W A S D</kbd><p>Move<small>double-tap left/right to run</small></p></div><div><kbd>J</kbd><p>Attack<small>tap after each strike</small></p></div><div><kbd>SPACE</kbd><p>Jump<small>J in air · J + Space: back attack</small></p></div><div><kbd>K</kbd><p>Magic<small>fill the meter with axe hits</small></p></div><div><kbd>P</kbd><p>Pause</p></div></footer><div className="footnote"><span>FORGED IN THE AGE OF SWORD & SORCERY</span><span>Headphones recommended ↗</span></div></main>}
