(() => {
  ...
  let fireTexture,smokeTexture,chickenAtlas;const deathFireTextures=[];let cinderFar;
  ...
  let displayedHealth=100,displayedMana=100;
  ...
  const environments=[];
  let nextId = 0,transition=null,bloodWipe=null,stageWalk=null;
  const wipeClose=.35/.49,wipeOpen=.45/.49,wipeHold=.15;
  ...
  function canCast() {
    return phase === 'playing' && magic >= 100 && !spell && !hero.air && !hero.attack && !hero.down && !hero.pickup;
  }
  function syncMagic() {
    const ready = canCast();
    $('magic-fill').style.width = magic + '%';
    $('magic-meter').setAttribute('aria-valuenow', String(magic));
    $('magic-meter').setAttribute('aria-valuetext', `${Math.ceil(magic)} percent. Press K at full charge for a 1.5 second screen-wide lightning burst. Charge with weapon hits and kills.`);
  ...
  function change(next) {
    if (!['playing','paused'].includes(next)) spell = null;
    phase = next;
    keys.clear();pressed.clear();
    $('overlay').setAttribute('data-phase',next);
    $('pause-actions').hidden=next!=='paused';
    $('title').hidden = next !== 'title';
    $('hud').hidden = false;
    $('overlay').style.backdropFilter=next==='lost'?'none':'';
    $('overlay').style.background=next==='lost'?'transparent':'';
    $('overlay').hidden = !['paused', 'won', 'lost'].includes(next);
    $('outcome').textContent = next === 'paused' ? 'CAIRN' : next === 'won' ? 'THE VALLEY IS FREE' : 'THE LEGION ENDURES';
    $('message').textContent = next === 'paused' ? 'The battle waits.' : next === 'won' ? 'A legend rises.' : 'Even heroes fall.';
  ...
  function spawn() {
    const nextBackground=environments[Math.floor(Math.random()*environments.length)];
    if(nextBackground!==background){blood.reset();aftermath.reset();}
  ...
  function start() {
    ...
    hero = make(470, 660, 100, true); hero.weapon=weaponId; wave = 1; score = 0; kills = 0; magic = 0;
    displayedHealth=hero.hp;displayedMana=magic;bloodWipe=null;combo = 0; sparks = []; shake = 0; spell = null; accumulator = 0;
    blood.reset();aftermath.reset();field.reset();
    encounters = E.plan();
    keys.clear(); pressed.clear(); change('playing'); spawn();beginStageWalk('enter');transition={age:wipeClose+wipeHold,swapped:true};
  }
  ...
  function damage(e, attack, attacker) {
    ...
    if(!isHero&&!attack.magic)e.hitGlow=.1;
    ...
    if (e.hp <= 0) {
      ...
      if (isHero) {bloodWipe={age:-.5,seed:Math.floor(Math.random()*2147483647)+1};stageWalk=null;change('dying');}
  ...
  function tickActor(f, dt) {
    f.hitGlow=Math.max(0,(f.hitGlow||0)-dt);
  ...
  function beginStageWalk(kind){
    stageWalk=kind;keys.clear();pressed.clear();spell=null;
    hero.attack=null;hero.air=null;hero.down=null;hero.pickup=null;
    hero.hurtTicks=hero.recovering=hero.recoil=0;hero.running=false;
    hero.dir=1;hero.moving=true;hero.velocityX=hero.velocityY=0;
    if(kind==='enter'){hero.x=-140;hero.y=660;}
  }
  function update(dt) {
    if(stageWalk){
      hero.x+=322*dt;hero.moving=true;hero.dir=1;hero.stride=(hero.stride+1.4*dt/(40*M.STEP))%1;
      keys.clear();pressed.clear();
      if(stageWalk==='enter'&&hero.x>=W/2){hero.x=W/2;hero.moving=false;stageWalk=null;}
      else if(stageWalk==='exit'&&hero.x>W+140){
        stageWalk=null;hero.moving=false;
        // The closing wipe already owns the level change.
      }
      return;
    }
    ...
    if(phase==='playing'){hero.casting=!!spell;const hp=hero.hp;field.step(dt,wave,hero,blood);if(hero.hp!==hp)sync();}
    ...
    if (!spell && !hero.pickup && pressed.has('j') && pressed.has(' ') && !hero.air) M.begin(hero, 'back');
    else if (!spell && !hero.pickup && pressed.has(' ')) {
      if (M.startJump(hero)) beep(160, .12, 'sine');
    } else if (!spell && !hero.pickup && pressed.has('j')) {
      if (M.begin(hero, M.selectStrike(hero, enemies))) beep(230, .15);
    }
    M.stepMotion(hero, spell||hero.pickup ? 0 : dx, spell||hero.pickup ? 0 : dy, spell||hero.pickup ? 0 : edge, bounds);
    ...
    if (phase === 'playing' && enemies.every(e => e.hp <= 0 && e.burnAge > 3.4)) {
      if(!transition&&!stageWalk){
        beginStageWalk('exit');
        transition={age:0,swapped:false,swap:()=>{
          if(wave===encounters.length){stageWalk=null;hero.moving=false;change('won');}
          else {wave++;spawn();beginStageWalk('enter');}
        }};
      }
    }
    syncMagic();
  ...
  function drawHUD() {
    ...
    gauge(81,displayedHealth,['#f59d91','#bf221e','#710807','#a41413'],false);
    gauge(221,displayedMana,['#d5f5ff','#269bff','#074891','#1585e2'],canCast());
  ...
  function drawFighter(f, isHero) {
    ...
    const p = isHero && hero.pickup && !hero.down && !hero.hurtTicks ? {atlas:'hero-pickup-unarmed-v1',frame:Math.min(7,Math.floor(hero.pickup.age/.0725))} : isHero && spell && !f.down && !f.hurtTicks ? {atlas:'hero-cast-unarmed-v1',frame:castFrame} : !isHero && f.kind!=='legion' ? E.pose(f) : pose(f, isHero), atlas = atlases[p.atlas];
    if (!atlas) return;
    ...
    shadow.addColorStop(0, '#030403b8'); shadow.addColorStop(.4, '#03040385'); shadow.addColorStop(.72, '#03040335'); shadow.addColorStop(1, '#03040300');
    ...
    // Invulnerability is a gameplay timer, not a repeated visual hit flash.
    else if(!isHero&&f.hitGlow>0)ctx.filter=`brightness(${1+.22*f.hitGlow/.1})`;
    const paint=g=>{
  ...
  function inStorm(e){return e.hp>0 && e.x>=0 && e.x<=W && e.y>=0 && e.y<=H;}
  function roar(){
  ...
  function drawSpell() {
    ...
    if(spell.age<77){
      if(spell.upwardCycle!==cycle){spell.upward=lightningChannel(93417+cycle*104729,500);spell.upwardCycle=cycle;}
  ...
  function render(dt) {
    ...
    displayedHealth+=(hero.hp-displayedHealth)*blend;
    displayedMana+=(magic-displayedMana)*blend;
    if(Math.abs(displayedHealth-hero.hp)<.05)displayedHealth=hero.hp;
    if(Math.abs(displayedMana-magic)<.05)displayedMana=magic;
    drawHUD();
    ctx.save(); ctx.clearRect(0, 0, W, H);
    if (!reducedMotion && phase !== 'paused') ctx.translate((Math.random() - .5) * shake, (Math.random() - .5) * shake);
    if (background) background.draw(ctx, clock, reducedMotion,hero.x/W-.5);
    ...
    else [...enemies, hero, ...aftermath.items, ...(field.chicken?[field.chicken]:[])].sort((a, b) => a.y - b.y).forEach(f => {
      if(f===field.chicken)field.draw(ctx,chickenAtlas);
      else if(f.owner){aftermath.equipment(ctx,f);if(!f.owner.player)aftermath.flames(ctx,f,deathFireTextures,smokeTexture);}
      else {drawFighter(f,f===hero);if(f!==hero&&f.hp<=0)aftermath.flames(ctx,f,deathFireTextures,smokeTexture);}
    });
  ...
  // An iris cutout in a black matte: close to a skull, hold, then reveal.
  function drawTransition(){
    if(!transition)return;const a=transition.age;
    const progress=a<wipeClose?1-a/wipeClose:a<wipeClose+wipeHold?0:(a-wipeClose-wipeHold)/wipeOpen;
    const scale=Math.exp(clamp(progress)*Math.log(61))-1;
    ctx.save();ctx.setTransform(1,0,0,1,0,0);ctx.fillStyle='#000';
    if(scale<.015){ctx.fillRect(0,0,W,H);ctx.restore();return;}
    const p=new Path2D();p.rect(0,0,W,H);
    const skull=new Path2D();
    skull.moveTo(-65,34);skull.lineTo(-99,20);skull.lineTo(-114,-9);skull.lineTo(-101,-35);
    skull.bezierCurveTo(-119,-65,-103,-108,-76,-122);skull.bezierCurveTo(-37,-143,35,-138,71,-125);skull.bezierCurveTo(105,-112,118,-69,104,-37);
    skull.lineTo(118,-12);skull.lineTo(102,19);skull.lineTo(64,34);skull.lineTo(55,56);
    skull.lineTo(48,58);skull.lineTo(43,91);skull.lineTo(32,74);skull.lineTo(28,58);skull.lineTo(19,61);skull.lineTo(17,81);skull.lineTo(6,84);skull.lineTo(4,64);skull.lineTo(-6,64);skull.lineTo(-9,85);skull.lineTo(-20,82);skull.lineTo(-21,60);skull.lineTo(-31,57);skull.lineTo(-37,80);skull.lineTo(-47,94);skull.lineTo(-51,58);skull.lineTo(-58,53);skull.closePath();
    // Centre the skull bounds on screen; zoom far enough for every opaque
    // detail to exit naturally without fading.
    const cy=H/2+25*scale;
    p.addPath(skull,{a:scale,b:0,c:0,d:scale,e:W/2,f:cy});ctx.fill(p,'evenodd');ctx.translate(W/2,cy);ctx.scale(scale,scale);ctx.globalAlpha=1;
    ctx.beginPath();ctx.moveTo(-89,-44);ctx.bezierCurveTo(-68,-48,-39,-32,-17,-25);ctx.lineTo(-27,1);ctx.bezierCurveTo(-46,15,-77,8,-83,-10);ctx.closePath();
    ctx.moveTo(87,-46);ctx.bezierCurveTo(62,-43,34,-30,16,-23);ctx.lineTo(28,3);ctx.bezierCurveTo(54,12,78,5,83,-15);ctx.closePath();ctx.fill();
    ctx.beginPath();ctx.moveTo(-2,-6);ctx.lineTo(-15,29);ctx.lineTo(-5,25);ctx.lineTo(2,31);ctx.lineTo(14,27);ctx.lineTo(5,-5);ctx.closePath();ctx.fill();ctx.restore();
  }

function drawBloodSplatter(ctx,state,W,H){
 const w=320,h=180,step=1/60;
 if(!state.splat){
  const canvas=document.createElement('canvas');canvas.width=w;canvas.height=h;
  const g=canvas.getContext('2d');let seed=state.seed;
  const random=()=>{seed=(Math.imul(seed,1664525)+1013904223)|0;return (seed>>>0)/4294967296;};
  // A few uneven impact clusters plus scattered arrivals; seeded per replay.
  const bursts=Array.from({length:5},()=>({at:.06+random()*.5,width:.012+random()*.07}));
  const deposits=[];
  for(let n=0;n<280;n++){
   const burst=bursts[Math.floor(random()*bursts.length)];
   const arrival=random()<.24?.03+random()*.62:Math.max(.03,Math.min(.65,burst.at+(random()+random()-1)*burst.width));
   deposits.push({x:random()*w,y:random()*h,
    rx:n<72?9+random()*19:1+random()*6,ry:n<72?6+random()*12:1+random()*4,
    angle:random()*Math.PI,phase:random()*Math.PI*2,lobes:2+Math.floor(random()*4),roughness:Math.pow(random(),1.7),
    amount:n<72?1.7+random()*2: .3+random()*1.1,at:arrival});
  }
  state.splat={canvas,g,pixels:g.createImageData(w,h),field:new Float32Array(w*h),next:new Float32Array(w*h),time:0,deposits,done:0};
 }
 const fade=clamp((state.age-.25)/1.3);
 ctx.save();ctx.globalAlpha=fade*fade*(3-2*fade);ctx.fillStyle='#700910';ctx.fillRect(0,0,W,H);ctx.restore();
 const f=state.splat;
 if(state.age<f.time-step){state.splat=null;return drawBloodSplatter(ctx,state,W,H);}
 while(f.time+step<=state.age+1e-7){
  for(const d of f.deposits){if(d.at>f.time||d.used)continue;d.used=true;
   const bound=Math.max(d.rx,d.ry)*2.7;
   for(let y=Math.max(0,Math.floor(d.y-bound));y<Math.min(h,d.y+bound);y++)for(let x=Math.max(0,Math.floor(d.x-bound));x<Math.min(w,d.x+bound);x++){
    const dx=x-d.x,dy=y-d.y,cs=Math.cos(d.angle),sn=Math.sin(d.angle);
    const u=(dx*cs+dy*sn)/d.rx,v=(-dx*sn+dy*cs)/d.ry;
    const theta=Math.atan2(v,u);
    const contour=1+d.roughness*(.22*Math.sin(theta*d.lobes+d.phase)+.085*Math.sin(theta*(d.lobes+3)-d.phase));
    const q=(u*u+v*v)/(contour*contour);
    f.field[y*w+x]+=d.amount*Math.exp(-q*2.8);
   }
  }
  const settling=clamp((f.time-1.8)/2);
  const flowRate=1-settling*settling*(3-2*settling);
  const a=f.field,b=f.next;b.set(a);
  for(let y=0;y<h;y++)for(let x=0;x<w;x++){
   const i=y*w+x,v=a[i],mobile=Math.max(0,v-.12);
   if(!mobile)continue;
   // Thick rivulets drain faster; capillary spreading merges adjacent deposits.
   const down=Math.min(mobile*.38,mobile*mobile*.075)*flowRate;
   b[i]-=down;if(y<h-1)b[i+w]+=down;
   if(x<w-1){const flux=Math.max(-.08,Math.min(.08,(v-a[i+1])*.025));
    const limited=(flux>0?Math.min(flux,mobile*.12):-Math.min(-flux,Math.max(0,a[i+1]-.12)*.12))*flowRate;
    b[i]-=limited;b[i+1]+=limited;
   }
  }
  f.field=b;f.next=a;f.time+=step;
 }
 if(f.drawn===f.time){ctx.drawImage(f.canvas,0,0,W,H);return;}
 const a=f.field,out=f.pixels.data;
 for(let y=0;y<h;y++)for(let x=0;x<w;x++){
  const i=y*w+x,v=a[i],k=i*4;
  const coverage=clamp((v-.035)*18);if(!coverage){out[k+3]=0;continue;}
  const nx=(a[y*w+Math.max(0,x-1)]-a[y*w+Math.min(w-1,x+1)])*2.2;
  const ny=(a[Math.max(0,y-1)*w+x]-a[Math.min(h-1,y+1)*w+x])*2.2;
  const inv=1/Math.sqrt(1+nx*nx+ny*ny);
  const diffuse=Math.max(0,(-nx*.4-ny*.5+.7)*inv);
  const highlight=Math.pow(Math.max(0,(-nx*.28-ny*.38+.881)*inv),65);
  const thick=1/(1+v*.6);
  out[k]=48+thick*54+diffuse*21+highlight*175;
  out[k+1]=2+diffuse*5+highlight*139;
  out[k+2]=7+diffuse*8+highlight*132;out[k+3]=coverage*255;
 }
 f.g.putImageData(f.pixels,0,0);f.drawn=f.time;ctx.drawImage(f.canvas,0,0,W,H);
}

  function drawBloodWipe(){if(bloodWipe&&bloodWipe.age>=0)drawBloodSplatter(ctx,bloodWipe,W,H);}

  function loop(now) {
    if (!running) return;
    const raw = last === null ? 0 : Math.min(.25, Math.max(0, (now - last) / 1000)); last = now;
    if(bloodWipe&&phase==='dying'){bloodWipe.age=Math.min(3.8,bloodWipe.age+raw);if(bloodWipe.age>=3.8&&hero.down?.ground)change('lost');}
    if(transition&&phase!=='paused'){transition.age+=raw;if(!transition.swapped&&transition.age>=wipeClose){transition.swapped=true;transition.swap?.();}if(transition.age>=wipeClose+wipeHold+wipeOpen)transition=null;}
    const frozen = ['paused', 'won', 'lost'].includes(phase);
    const dt = frozen ? 0 : raw*(phase==='dying'&&hero.death<1.3?.3:1);
    clock += dt;
    if (frozen || !ready || (transition && !stageWalk)) accumulator = 0;
    else {
      accumulator += dt;
    ...
    render(dt);drawTransition();drawBloodWipe(); raf = requestAnimationFrame(loop);
  }
  ...
  const down = event => {
    ...
    if(key==='escape'){
      ...
      if(phase==='title')$('options-back').click?.();
      else if(phase==='playing')change('paused');else if(phase==='paused')change('playing');
      return;
    }
    if(['title','paused','won','lost'].includes(phase)){
      const root=phase==='title'?$('title'):$('overlay');
      const buttons=Array.from(root.querySelectorAll?.('button')||[]).filter(b=>!b.disabled&&b.getClientRects().length);
      if(['arrowdown','arrowup','w','s','tab'].includes(key)){
        event.preventDefault();if(!buttons.length)return;
        const index=buttons.indexOf(document.activeElement),back=key==='arrowup'||key==='w'||(key==='tab'&&event.shiftKey);
        buttons[(index+(back?-1:1)+buttons.length)%buttons.length].focus();return;
      }
      if(key==='enter'||key===' '){event.preventDefault();if(!event.repeat)(buttons.find(b=>b===document.activeElement)||buttons[0])?.click();return;}
      if(key!=='p')return;
    }
  ...
  $('start').onclick = start;
  $('pause-sound').onclick=()=>{$('sound').click();$('pause-sound').textContent=muted?'SOUND OFF':'SOUND ON';};
  $('return-title').onclick=()=>{transition=null;stageWalk=null;bloodWipe=null;change('title');};
  ...
  const names = ['hero-pickup-unarmed-v1','hero-cast-unarmed-v1', 'hero-reactions-unarmed-v8', 'hero-walk-unarmed-v8', 'hero-actions-unarmed-v8', 'hero-extra-unarmed-v8', 'hero-close-unarmed-v8', 'enemy-walk-v4', 'enemy-attack-v4', 'enemy-charge-v5', 'enemy-combat-v3', 'enemy-bone-v1', 'enemy-shield-v1', 'enemy-marauder-v1', 'enemy-champion-v1'];
  const atlasConfig = {
    ...
    'hero-pickup-unarmed-v1':{columns:4,rows:2,frames:8},
  ...
  Promise.all([
    ...
    ...Array.from({length:4},(_,i)=>loadImage('/art/fluid-fire-v2-'+i+'.png').then(image=>{deathFireTextures[i]=image;})),
    loadImage('/art/cinder-concept-v5.png').then(image=>{cinderFar=image;}),
    ...['valley.png','swamp-concept-v4.png','cinder-concept-v6.png'].map((file,index)=>loadImage('/art/'+file).then(image=>{environments[index]={image,key:['valley','swamp','cinder'][index]};})),
    ...names.map(name => loadImage(`/art/${name}.png`).then(image => {
      atlases[name] = new Atlas(decodeChroma(image), atlasConfig[name]);
    })),
  ]).then(() => {
    if (!running) return;
    environments.forEach((e,i)=>{environments[i]=new window.AshenEnvironments.Environment(e.image,e.key,W,H,smokeTexture,fireTexture,e.key==='cinder'?cinderFar:null);});
    background=environments[0];
    ...
    ready = true; $('start').disabled = false; $('start').textContent = 'BEGIN';
  }).catch(error => {
  ...
  window.stopGame = () => {
    ...
    environments.forEach(e=>e.dispose?.()); audio?.close();
  };
  window.ashenAxe = {
    status: () => ({ phase, environment:background?.key, health: Math.round(hero.hp), wave, score, magic, magicMax: 100, magicReady: canCast(), channeling: !!spell, weapon: weaponId, kills,
