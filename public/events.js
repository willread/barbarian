 ...
 class FieldEvents{
  ...
  step(dt,wave,hero,blood){
   ...
   if(c.roast){
    ...
    if(hero.hp>0&&!hero.down&&!hero.air&&Math.abs(hero.x-c.x)<45&&Math.abs(hero.y-c.y)<26){hero.hp=hero.max;this.chicken=null;}return}
  ...
  draw(ctx,atlas){const c=this.chicken;if(!c||!atlas)return;ctx.save();ctx.translate(c.x,c.y-c.height*M.SCALE);ctx.scale(c.dir,1);atlas.paint(ctx,c.roast?10:c.hop?9:Math.floor(c.age*16)%8,132,1);ctx.restore()}
 }
 class Aftermath{
  ...
  flames(ctx,f,fire,smoke){
   ...
   const life=.15+f.engulf+.6,fade=1-clamp((f.burnAge-life+.25)/.25),s=f.size||1;
   ...
   for(const p of b.points){const frame=Math.floor((f.burnAge*30+p.offset)%64),sx=frame%8*128,sy=Math.floor(frame/8)*192,w=155*p.scale*s,h=230*p.scale*s;
