...
$kitRoot = 'E:\Cairn-build-tools\itch-page-kit'
...
Copy-Item -LiteralPath "$PSScriptRoot\assets\panel-stone.png" -Destination "$kitRoot\cairn-stone-background.png"
...
$recipe.images.background = 'cairn-stone-background.png'
...
$zipPath = 'E:\Cairn-build-tools\cairn-itch-page-kit.zip'
Compress-Archive -Path "$kitRoot\*" -DestinationPath $zipPath -Force
Add-Type -AssemblyName System.IO.Compression.FileSystem
$archive = [System.IO.Compression.ZipFile]::OpenRead($zipPath)
try {
  $files = @($archive.Entries | Where-Object { $_.Length -gt 0 })
  if ($files.Count -ne 9) { throw "Expected 9 nonempty files, found $($files.Count)" }
