...
Latest navigation/records refinement: Rise Again is first and initially selected; returning from Hall focuses its second menu item. Hall Back uses the title menu's full 1.0 lettering scale and reserves space below the scrolling list. Results show a single personal-best comparison with a signed parenthesized difference, including +0 for first/tied scores. Rankings and all personal stat bests are scoped by episode; episode 1 retains `citadel-v1` to preserve existing saves, while additional episodes use their own versioned keys. Only episode 1 is currently playable. Regression tests verify independent episode baselines and persistence.

Latest refinement removes the defeat headline and uses its space for a 470px-high panel beginning 28px from the top of the 16:9 composition. Subtle horizontal stat-row rules are restored. Small stone labels use a thicker glyph mask before the existing material/bevel pass; dynamic comparison/detail copy uses a local emboldened font variation. HUD area/wave/boss labels receive the same mask treatment, with a bake-version check so cached assets are refreshed. The victory headline is retained. These choices supersede the earlier defeat-heading and divider notes below.

Current presentation fits a centered 1440×810 (16:9) composition uniformly into every window; results no longer reflows into a portrait panel. The footer uses the existing compact-pause stack geometry and spacing, fitted to a larger 276px-high region. Its landing offsets still survive resizing. The heading is bright ivory, section headings warm gold, and values pale ivory/cool stone. A black overlay fades in over 0.8 seconds, with additional gradual darkening behind the menu; the blood stays visible. These choices supersede the responsive portrait and earlier footer notes below. Native tests cover uniform aspect fitting and centered stack bounds, and both exports are updated.
