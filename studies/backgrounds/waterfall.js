export async function createWaterfall(image,regions,width,height){
 ...
 function regionMask(polygon,blur,exclusions=[]){const mask=document.createElement('canvas');mask.width=width;mask.height=height;const ctx=mask.getContext('2d');ctx.fillStyle='black';ctx.fillRect(0,0,width,height);ctx.filter=`blur(${blur}px)`;ctx.fillStyle='white';ctx.beginPath();polygon.forEach(([x,y],i)=>i?ctx.lineTo(x*width,y*height):ctx.moveTo(x*width,y*height));ctx.closePath();ctx.fill();ctx.filter="none";ctx.fillStyle="black";for(const hole of exclusions){ctx.beginPath();hole.forEach(([x,y],i)=>i?ctx.lineTo(x*width,y*height):ctx.moveTo(x*width,y*height));ctx.closePath();ctx.fill()}return mask}
 const mask=regionMask(regions[0].polygon,3,regions[0].exclusions||[]),fire=regionMask(regions[1]?.polygon||[],10);
