...
## Native chapter integration
World 1 is The Fallen Citadel. Begin opens chapter selection; The Sunken Wilds and The Ashen Depths are disabled future chapters. Existing randomized encounters are grouped 2/2/2/2 across aqueduct, gate, foundry and keep; each area has two regular waves, and the keep finishes with an additional boss encounter (nine encounters total). Only scene changes trigger the walk/skull transition. `tools/godot/bake-world.mjs` runs during the standard build, copying source paintings/loop atlases and baking foreground cutouts into native assets. `godot/worlds/citadel.json` retains walk polygons. The native renderer plays the two-second loops with adjacent-frame interpolation and composites foreground above actors. Screen two uses the user's latest exported geometry. No publishing required.
