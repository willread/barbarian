import {screens,duration,assetFiles,prepareAssets,drawScenery,drawAtmosphere,drawForeground,drawActor} from './scene.js?revision=2';
...
let selected=0,time=0,running=!matchMedia('(prefers-reduced-motion: reduce)').matches,last=performance.now(),enabled=[true,true,true],actor={x:.5,y:.85};
...
const previous=await Promise.all(['01-crossing','02-hollow','03-procession','04-throne'].map(name=>load('../swamp-v1/'+name+'.png')));
...
function select(i){selected=i;time=0;enabled=[true,true,true];document.querySelectorAll('nav button').forEach((b,j)=>b.setAttribute('aria-pressed',String(i===j)));$('name').textContent=screens[i].name;$('description').textContent=screens[i].mood+' '+screens[i].motion;$('effects').replaceChildren();screens[i].effects.forEach((name,j)=>{const l=document.createElement('label'),c=document.createElement('input');c.type='checkbox';c.checked=true;c.onchange=()=>enabled[j]=c.checked;l.append(c,' '+name);$('effects').append(l)});history.replaceState(null,'','#'+(i+1));draw()}
function draw(){
  const still=$('still').checked,frameTime=still?0:time;
  drawScenery(g,selected,images[selected],assets,frameTime,still?[false,false,false]:enabled);
