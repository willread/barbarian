...
func enemy_frame(e: Dictionary) -> int:
	if e.kind in ["witch","bearer","king","saint"]:
		...
		if not e.attack.is_empty():return 3 if e.attack.age<e.attack.from else 4 if e.attack.age<=(e.attack.to+15 if e.attack.type=="hellScream" else e.attack.from+8) else 5
