...
func check():
	...
	for i in 22:records.finish(sample("run-%d"%i,300+i))
	var low=records.finish(sample("low",1,90))
	assert(low.rank==0 and "best_combo" in low.new_stats and records.board().runs.size()==20)
	...
	assert(loaded.board().runs.size()==20 and loaded.board().bests.best_combo==90)
	...
	file.close()
	assert(CairnRunRecords.new(path).board().runs.size()==20)
