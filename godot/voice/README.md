...
Gameplay dialogue now uses one slot per scene across all three waves, plus one separate slot for the boss encounter, consumed when playback starts even if interrupted. Spoken IDs remain unavailable until a new run; pause, area changes and voice resets retain this history. Queued lines expire or are dropped on wave changes without consuming their ID.
