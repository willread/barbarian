...
const variants=[
 {id:'deep-gurgle',events:72,slurp:9,pitch:.58,seed:811,gulp:.12},
 {id:'wet-slurp',events:90,slurp:3,pitch:.8,seed:191,gulp:.35},
 {id:'bubble-boil',events:180,slurp:13,pitch:1.3,seed:632,gulp:.04},
 {id:'thick-suction',events:55,slurp:2,pitch:.45,seed:947,gulp:.28}
];
for(const v of variants){
// Seeded, periodic liquid Foley: descending bubble resonances and wet suction bursts.
const rate=44100,seconds=8,n=rate*seconds,out=new Float64Array(n);let seed=v.seed;
const random=()=>{seed=(Math.imul(seed,1664525)+1013904223)>>>0;return seed/4294967296};
for(let e=0;e<v.events;e++){
 const start=Math.floor(random()*n),slurp=e%v.slurp===0,duration=slurp?.35+random()*.45:.06+random()*.18;
 ...
 for(let j=0;j<duration*rate;j++){
  ...
  phase+=2*Math.PI*f*v.pitch*(.3+.7*Math.exp(-u*5))/rate;
...
const temp='E:/Cairn-build-tools/mire-'+v.id+'.wav';fs.writeFileSync(temp,wav);
execFileSync('ffmpeg',['-hide_banner','-loglevel','error','-y','-i',temp,'-stream_loop','-1','-i','godot/audio/gulp.ogg','-filter_complex',`[0:a]lowpass=f=2600[a];[1:a]asetrate=32000,aresample=44100,lowpass=f=1300,volume=${v.gulp}[b];[a][b]amix=inputs=2:duration=first:normalize=0,alimiter=limit=0.85,afade=t=in:d=0.015,afade=t=out:st=7.985:d=0.015[out]`,'-map','[out]','-t','8','-c:a','libvorbis','-q:a','5','studies/backgrounds/mire-v2/'+v.id+'.ogg'],{windowsHide:true});
console.log('Baked '+v.id);
}
