...
for(let i=0;i<4;i++){
 const start=render(i,0),end=render(i,24);let delta=0;for(let k=0;k<start.length;k++)delta+=Math.abs(start[k]-end[k]);assert(delta/start.length<.01,'loop endpoints');
 assert.notDeepEqual(start,render(i,2.3),'scene must animate');
 for(let e=0;e<screens[i].effects.length;e++){const on=render(i,2.3);const enabled=[true,true,true];enabled[e]=false;assert.notDeepEqual(on,render(i,2.3,enabled),'effect toggle '+i+':'+e)}
 render(i,2.3);fs.writeFileSync(root+'review-'+(i+1)+'.png',c.toBuffer('image/png'));sg.drawImage(await loadImage(c.toBuffer('image/png')),i%2*640,Math.floor(i/2)*360,640,360);
