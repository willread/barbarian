...
stone=stone.replace('g.fillText(ch,x,pad*scale);','if(el.dataset.boldSmall){g.strokeStyle="#fff";g.lineJoin="round";g.lineWidth=size*scale*.028;g.strokeText(ch,x,pad*scale);}g.fillText(ch,x,pad*scale);');
...
for(const label of labels){
 ...
 if(!small&&manifest.menu["HUD "+label]&&fs.existsSync(path.join(out,`menu-${manifest.menu["HUD "+label].id}.png`))&&fs.existsSync(path.join(out,`menu-${manifest.menu["HUD "+label].id}-fuel.png`)))continue;
 const el={textContent:label,getClientRects:()=>[1],dataset:{stoneFont:'Cinzel',boldSmall:small},computed:{fontSize:'75',letterSpacing:'1'},querySelector:()=>true,classList:{add(){}},style:{setProperty(){}}};sc.window.bakeStone(el);const r=el._stoneFrames,id='hud-'+label.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/-$/,'');
