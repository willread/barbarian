...
export function splitEpisodeSheet(image,kind){
 ...
 for(let start=0;start<labels.length;start++){
  ...
  while(stack.length){
   ...
   for(const q of [x?p-1:-1,x<w-1?p+1:-1,p-w,p+w])if(q>=0&&q<labels.length&&!labels[q]&&d[q*4+3]>=128){if(kind==='king'&&kingSide(p)!==kingSide(q))continue;if(kind==='warden-v2'&&wardenSide(p)!==wardenSide(q))continue;labels[q]=id;stack.push(q)}
  }
  parts.push({id,points,x0,y0,x1,y1});
 }
 const count=kind.startsWith('warden')?16:kind==='king-roots'?4:8;
 ...
 const row=p=>kind.startsWith('warden')?Math.floor((p.y0+p.y1)*.5/(h/4)):['king-polish','king-roots'].includes(kind)?Number((p.y0+p.y1)*.5>=h/2):Number(p.y0>=h/2);
 ...
 for(let pass=0;pass<6;pass++){
  const expanded=[];
  for(const p of frontier){const x=p%w;for(const q of [x?p-1:-1,x<w-1?p+1:-1,p-w,p+w])if(q>=0&&q<labels.length&&!labels[q]&&d[q*4+3]>1){if(kind==='king'&&kingSide(p)!==kingSide(q))continue;if(kind==='warden-v2'&&wardenSide(p)!==wardenSide(q))continue;labels[q]=labels[p];expanded.push(q)}}
  frontier=expanded;
 }
 return figures.map((part,i)=>{
  ...
  const cw=Math.floor(w/columns),ch=Math.floor(h/(kind.startsWith('warden')?4:2)),flip=kind==='witch'&&i<2;
