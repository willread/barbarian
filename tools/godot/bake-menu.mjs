...
if(!process.env.MENU_BATCH){
 for(let i=1;i<=24;i++){const r=spawnSync(process.execPath,['--expose-gc',process.argv[1]],{env:{...process.env,MENU_BATCH:String(i)},stdio:'inherit',windowsHide:true});if(r.status!==0)process.exit(r.status||1);}
 process.exit(0);
}
...
const labels=[...Array.from({length:10},(_,i)=>String(i)),...Array.from({length:8},(_,i)=>`${i+1}X`),...Array.from({length:4},(_,i)=>`AREA ${i+1}/4`),...Array.from({length:3},(_,i)=>`WAVE ${i+1}/3`),'BOSS','CONTROLS','HOLIDAY: OFF','HOLIDAY: CHRISTMAS','HOLIDAY: HALLOWEEN','GAME',...['GRAVECLEAVER','BLACKTOOTH','BARROW STAR','GATEBREAKER'].map(x=>'WEAPON: '+x),'EP 1: THE FALLEN CITADEL','EP 2: THE SUNKEN WILDS','EP 3: THE ASHEN DEPTHS','BEGIN','OPTIONS','SOUND','DISPLAY','MUSIC: ON','MUSIC: OFF','VOICE: ON','VOICE: OFF',...Array.from({length:101},(_,i)=>`VOLUME: ${i}`),'SOUND: ON','SOUND: OFF','FULLSCREEN: ON','FULLSCREEN: OFF','BACK','RETURN TO BATTLE','QUIT TO TITLE','QUIT','RISE AGAIN','THE BATTLE WAITS.','EVEN HEROES FALL.','A LEGEND RISES.'];
