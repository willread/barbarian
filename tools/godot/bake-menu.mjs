...
if(!process.env.MENU_BATCH){
 for(let i=1;i<=20;i++){const r=spawnSync(process.execPath,['--expose-gc',process.argv[1]],{env:{...process.env,MENU_BATCH:String(i)},stdio:'inherit',windowsHide:true});if(r.status!==0)process.exit(r.status||1);}
 process.exit(0);
}
