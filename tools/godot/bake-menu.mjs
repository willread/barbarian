...
if(!process.env.MENU_BATCH){
 for(let i=1;i<=16;i++){const r=spawnSync(process.execPath,['--expose-gc',process.argv[1]],{env:{...process.env,MENU_BATCH:String(i)},stdio:'inherit',windowsHide:true});if(r.status!==0)process.exit(r.status||1);}
 process.exit(0);
}
...
const labels=['BEGIN','OPTIONS','SOUND','DISPLAY','MUSIC: ON','MUSIC: OFF','VOICE: ON','VOICE: OFF',...Array.from({length:101},(_,i)=>`VOLUME: ${i}`),'SOUND: ON','SOUND: OFF','FULLSCREEN: ON','FULLSCREEN: OFF','BACK','RETURN TO BATTLE','QUIT TO TITLE','QUIT','RISE AGAIN','THE BATTLE WAITS.','EVEN HEROES FALL.','A LEGEND RISES.'];
