...
for(const text of (process.argv.includes('--hall')?['HALL OF LEGENDS','HALL OF','LEGENDS']:['CONTROLS','Move','Attack','Jump','Magic','Run','Spin','Charge','Jump attack','Pause','HALL OF LEGENDS','HALL OF','LEGENDS'])) {const r=await lettering(text);const c=createCanvas(r.w,r.h);c.getContext('2d').drawImage(r.img,r.x,r.y,r.w,r.h,0,0,r.w,r.h);fs.writeFileSync(path.join(out,text.toLowerCase().replaceAll(' ','-')+'.png'),c.toBuffer('image/png'));}
