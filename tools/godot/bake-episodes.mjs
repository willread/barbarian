...
import {screens,lane,assetFiles,prepareAssets,drawScenery,drawAtmosphere,drawForeground} from '../../studies/backgrounds/swamp-v3/scene.js';
const out='godot/assets/',source='asset-sources/art/episodes/';
const stamp=out+'episodes-bake.json';
const inputs=[import.meta.filename,'tools/godot/bake-swamp-loop.mjs','tools/godot/split-episode-sheet.mjs',...fs.readdirSync(source).map(f=>source+f),...fs.readdirSync('studies/backgrounds/ashen-v1').map(f=>'studies/backgrounds/ashen-v1/'+f),...['scene.js',...screens.map(s=>s.file),...Object.values(assetFiles)].map(f=>'studies/backgrounds/swamp-v3/'+f)];
...
function worker(key,area,rect,mode){
 const result=spawnSync(process.execPath,['tools/godot/bake-swamp-loop.mjs',key,String(area),JSON.stringify(rect),mode],{stdio:'inherit',windowsHide:true});
 if(result.error||result.status!==0)throw Error('Swamp atlas bake failed: '+key+' ('+result.status+')');
}
worker('mask',0,[],'mask');
function bakeLoop(key,rect,area,foreground=false,still=false){
 const [x,y,w,h]=rect,fw=w,fh=h,count=still?1:288,columns=still?1:12,rows=still?1:24;
 const file=key+'.png';worker(key,area,rect,still?'rocks':'detail');
...
for(let i=0;i<4;i++){
 ...
 const regions=rects[i].map((rect,j)=>bakeLoop(key+'-v3-detail-'+j,rect,i));
 if(i===3)regions.push(bakeLoop(key+'-v4-rocks',[0,590,1280,130],i,true,true));
 swamp.push({key,title:screens[i].name,revision:4,birds:i===0,regions,walkable:{polygon:[[0,lane.top],[1,lane.top],[1,lane.bottom],[0,lane.bottom]]},movement_y:[lane.top*810,lane.bottom*810],framing:{bottom_crop:.03}});
}
